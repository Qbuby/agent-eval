...
- [ ] M0 is locally complete except for external review: the official 0.0.11 PyPI wheel was downloaded from its pinned URL, verified against SHA-256 `ab9473092cac37e4f00347cd92d9e629424bcba99731c8d082b8031a29d6fbdf`, and exercised on Linux with CPython 3.12.13. The three reviewed data tools, success, business error, timeout, malformed JSON, and token non-disclosure contracts pass. A reviewed upstream commit and unambiguous written license approval remain unavailable.
...
- [ ] M3 partially complete: the independent non-Axi analysis runtime and the license-gated Axi runtime Dockerfile exist and have protocol tests; the Axi image cannot be built or smoked until the reviewed 0.0.11 artifact is supplied.
- [ ] M4 partially complete: SandboxTemplates, zero-size warm pools, namespaced RBAC, ServiceAccount, internal Service, NetworkPolicy, immutable-image rendering, local patch validation, and a fail-closed staging acceptance tool exist; the tool's server-side dry-run and live isolation/lifecycle checks still require a real staging context.
...
- [ ] M6 partially complete: local tests cover token secrecy, cross-tenant predicates, cancellation, timeout, output/path limits, claim cleanup, and the complete staging smoke command flow. The live tool can verify executor-authenticated claim creation, Pod hardening, backend health, Internet and peer denial, controller TTL deletion, and strict cleanup; it has not run against staging, and browser two-tenant acceptance remains.
...
The local control plane, governed data surface, overlay tools, Kubernetes analysis adapter, static deployment resources, tenant rollout gates, reversible deployment patches, and staging acceptance automation now exist. The current execution-plane suite reports `106 passed, 1 skipped`; the deployment collection reports `43 passed`. `kubectl v1.34.1 patch --local` proves the ServiceAccount and environment round trip without losing unrelated configuration or product-plane reads. Deployment tests additionally prove exact recovery-state matching, fail-before-mutation lifecycle and input validation, and a prohibition on restoring `omniagent-executor`. The staging tool has read-only and explicitly confirmed live modes covering server dry-run, least-privilege RBAC, exact template/network/service state, executor-authenticated Claim creation, actual Pod hardening, backend health, Internet and peer isolation, controller TTL deletion, and strict cleanup. No execution capability has been enabled and no cluster object has been changed. Production readiness is still unproven until the Axi 0.0.11 license/artifact gate, immutable image build and scan, and actual staging/browser two-tenant acceptance are complete.
