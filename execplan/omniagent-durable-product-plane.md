...
- [ ] P5 partially complete: the separate Axi native client package, seven reviewed skills, license-gated runtime image, and fail-closed Kubernetes staging manifests exist; production enablement remains blocked by the missing Axi license artifact and unavailable approved image digests.
- [ ] P6 partially complete: migration 0043, focused tests, frontend build, WSL Docker rebuild, and Compose health checks pass; Kubernetes server-side dry-run and production sandbox E2E remain unavailable because no staging cluster was applied.
...
The Agent Eval side of the Kubernetes analysis runner is now complete and locally accepted. Each job attempt receives one DNS-safe claim identity; fixed scripts and inputs are uploaded without interpolating user content into commands; outputs are enumerated by a fixed helper and revalidated during download; claim cleanup covers success, failure, timeout, and cancellation. The worker distinguishes infrastructure retry from deterministic failure, and the internal submit route uses the same fail-closed readiness check as the runner factory. The current execution-plane regression reports `106 passed, 1 skipped`; the deployment collection reports `43 passed`, and Ruff, compilation, TOML parsing, and whitespace checks pass. A real `kubectl v1.34.1 patch --local` round trip proves enable, stop, cleanup, and re-enable preserve unrelated containers and settings, restore the original ServiceAccount, remove stale restoration evidence, and keep the product plane available. `deploy/k8s/omniagent-execution/staging_smoke.py` now provides read-only and explicitly confirmed live acceptance for CRDs, server dry-run, RBAC, exact template/network/service state, actual Pod hardening, backend health, network isolation, controller TTL deletion, and strict claim cleanup. Its local contracts pass, but no cluster was contacted because this machine has no current context; real staging and browser two-tenant E2E remain external P5/P6 gates.
...
From `D:/program/agent_eval`, create `.codex_tmp` and run `python -m pytest tests/test_omniagent_runtime tests/test_api/test_omniagent_chat.py tests/test_deployment -q --basetemp .codex_tmp/pytest-final`. The current result is `106 passed, 1 skipped`; the two warnings are caused by a pre-existing 31-byte test HMAC key. The deployment-only collection reports `43 passed`. Run `python -m ruff check src/agent_eval/omniagent_runtime src/agent_eval/omniagent_data src/agent_eval/api/routers/omniagent.py src/agent_eval/api/routers/omniagent_data.py src/agent_eval/api/routers/omniagent_internal.py src/agent_eval/api/routers/omniagent_product.py deploy/k8s/omniagent-execution/staging_smoke.py tests/test_omniagent_runtime tests/test_api/test_omniagent_chat.py tests/test_deployment`, `python -m compileall -q src/agent_eval packages/agent-eval-axi-tools deploy/omniagent/analysis-runtime deploy/omniagent/overlay deploy/k8s/omniagent-execution/staging_smoke.py`, and `git diff --check`. Build the backend with `--build-arg INSTALL_KUBERNETES_RUNNER=1` only for reviewed Kubernetes deployments. Do not apply production resources automatically.
